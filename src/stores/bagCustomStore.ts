...
interface BagCustomStore {
  ...
  // 마지막으로 적용 완료된 가방 패치 상태임
  appliedPatches: PlacedPatch[];

  // 현재 편집 대상으로 선택된 패치 id임
...
export const useBagCustomStore = create<BagCustomStore>((set) => ({
  ...
  addDraftPatch: (patch) =>
    set((state) => {
      ...
      // 이미 가방 위에 존재하면 새로 추가하지 않고 해당 패치만 선택함
      if (existingPatch) {
        return {
